...
![](images/pipe_magritte.png){width=13%}  Le tidyverse fonctionne grâce au **pipe** (%>%) contenu dans le package `magrittr`, qui permet d'appliquer un certain nombre de fonctions à une base de données. Le code est alors plus lisible grâce à une structuration des séquences d'opérations de **gauche à droite** (et non de l'extérieur à l'intérieur comme en R-base), 
...
Comme dit précédemment, un *package* contient un certain nombre de **fonctions** qui permettent de faire toutes sortes de manipulations sur des bases de données ou autres. Pour les utiliser il faut d'abord les installer grâce à la fonction `install.packages("nom_package")`, puis les appeler pour pouvoir les utiliser via la fonction `library(nom_package)`. 
...
- nombre de NA, les supprimer, les remplacer (moyenne, mode...)
