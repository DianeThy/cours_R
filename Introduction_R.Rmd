...
L'intérêt de l'utilisation des projets R réside aussi dans le fait de les déposer sur la plateforme de dépôt **[github](https://github.com/)**. Cela est possible en reliant RStudio à github, vous trouverez un tutoriel de configuration [à cette adresse](https://happygitwithr.com/rstudio-git-github.html).
...
Les **graphiques en barres** permettent de représenter un montant, une répartition par variable catégorielle (groupes). Ici on regarde le nombre d'observations (fleurs) de la base dans chaque espèce. Sachant qu'il y en a 50 par espèce, la représentation n'est pas très pertinente. On voit cependant en deuxième graphique la même visualisation avec les barres horizontales, ce qui est possible grâce à la fonction `coord_flip()` que l'on ajoute à la suite du code existant. 
